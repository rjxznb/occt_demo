...
test('AI concept styles provide blue liquid glass, editing glow, motion fallback, and narrow layout', async () => {
    ...
    assert.match(
        css,
        /\.ai-view-card-toolbar\s+\[data-action="delete"\]:hover:not\(:disabled\),\s*\.ai-view-card-toolbar\s+\[data-action="delete"\]:focus-visible\s*\{[\s\S]*?color:\s*#b4232f[\s\S]*?background:\s*rgba\(210,\s*53,\s*63,\s*0\.14\)/,
    );
    assert.match(
        css,
        /\.ai-concept-edit-controls\s*\{[\s\S]*?left:\s*50%[\s\S]*?right:\s*auto[\s\S]*?bottom:\s*18px[\s\S]*?transform:\s*translateX\(-50%\)/,
    );
    assert.match(
        css,
        /@media\s*\(max-width:\s*760px\)[\s\S]*?\.ai-concept-edit-controls\s*\{[\s\S]*?left:\s*12px[\s\S]*?right:\s*12px[\s\S]*?bottom:\s*12px[\s\S]*?transform:\s*none/,
    );
