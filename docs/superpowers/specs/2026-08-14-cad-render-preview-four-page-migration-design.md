...
先把 `occt_demo` 当前可用的 3D、全景和局部示意图 Web 产物迁移到 `cad_plugin` 的渲染预览窗口；完成 CAD 四页面装载验证后，再继续补齐 AI 局部示意图的任务进度、结果展示和端到端链路。
...
CAD 页面迁移和基础装载验证完成后，继续完成现有 AI 生图计划的剩余工作：
...
首轮 CAD 迁移不以真实生图链路完成为前置条件，但局部示意图页必须能够正常加载当前已有的视角、缩略图、小地图与生图条件 UI。后续补齐任务链路时，没有真实 API Key 不伪造成功结果；离线测试使用受控的假图像客户端验证完整状态流。
